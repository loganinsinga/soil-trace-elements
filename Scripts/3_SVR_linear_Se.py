...
OUTPUT_DIR = (
    r"C:\Users\logan\OneDrive\Documents\Publications\soil trace elements V2\SVR_Se"
)
...
def setup_logging():
    ...
    file_handler = logging.FileHandler(
        f"{log_file_loc}\\3_SVR_Se_{EXECUTION_ID}.log", "w"
    )
...
def get_predictors(master_table: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    extreme_valid_indxs = (
        predictors_extreme.index.tolist()
...
def plot_current_obs_vs_pred(element_data: pd.Series, pred_current_avg: pd.Series):
    ...
    fig.savefig(
        os.path.join(OUTPUT_DIR, f"{EXECUTION_ID}_obs_vs_pred.png"),
        bbox_inches="tight",
        dpi=1000,
    )
