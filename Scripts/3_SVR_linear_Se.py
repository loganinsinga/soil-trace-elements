...
def setup_logging():
    ...
    file_handler = logging.FileHandler(f"{log_file_loc}\\3_SVR_Se_{EXECUTION_ID}.log", "w")
...
def get_predictors(master_table: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    extreme_valid_indxs = predictors_extreme.index.tolist()  # get indexs associted with valid rows
...
def tune_model(element_data: pd.Series, predictors_current: pd.DataFrame) -> dict:
    ...
    tuning_results: pd.DataFrame = pd.DataFrame(data=tuning_results_storage, columns=["RMSE", "C", "epsilon"])
...
def plot_current_obs_vs_pred(element_data: pd.Series, pred_current_avg: pd.Series):
    "Plots observed verses the avg. of the predicted current data"

    LOGGER.info("Plotting observed vs avg. predicted for current data")

    rmse = sqrt(mean_squared_error(element_data, pred_current_avg))
    one_to_one = [element_data.min(), element_data.max()]

    fig, ax = plt.subplots()
    ax.plot(element_data, pred_current_avg, ".b", markersize=12)  # plot data
    ...
    ax.text(60, 1000, "RMSE = " + str(rmse), fontsize=12)
    ax.loglog()

    fig.savefig(os.path.join(OUTPUT_DIR, f"{EXECUTION_ID}_obs_vs_pred.png"), bbox_inches="tight", dpi=1000)
...
def main():
    ...
    LOGGER.info("Running machine learning - SVR Selenium")
