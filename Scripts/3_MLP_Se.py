...
OUTPUT_DIR = r"Processing\_3_MLP_Se"
...
def setup_logging():
    ...
    file_handler = logging.FileHandler(
        os.path.join(OUTPUT_DIR, f"_3_MLP_Se_{EXECUTION_ID}.log"), "w"
    )
...
def get_predictors(master_table: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    predictors_extreme_Sedep = master_table[
        [
            "Avg_CIA",
            "Avg_PH_CAC",
            "CLYPPT_M_s",
    ...
    predictors_extreme_Sedep = predictors_extreme_Sedep.loc[extreme_valid_indxs, :]
    ...
    predictors: dict[str, pd.DataFrame] = {
        ...
        "extreme_Sedep": predictors_extreme_Sedep,
...
def tune_model(element_data: pd.Series, predictors_current: pd.DataFrame) -> dict:
    ...
    hidden_layer_sizes_ = np.linspace(1, 51, 50, endpoint=True, dtype=int)
    alpha_ = np.geomspace(0.00000001, 100, num=1000, endpoint=True)
    power_t_ = np.geomspace(0.00000001, 100, num=1000, endpoint=True)
    beta_1_ = np.geomspace(0.00000001, 0.99999999, num=1000, endpoint=True)
    beta_2_ = np.geomspace(0.00000001, 0.99999999, num=1000, endpoint=True)
    epsilon_ = np.geomspace(0.0000000001, 100, num=10000, endpoint=True)
    ...
    for i in range(0, num_combos):
        ...
        hidden_layer_sizes = random.choice(hidden_layer_sizes_)
        alpha = random.choice(alpha_)
        power_t = random.choice(power_t_)
        beta_1 = random.choice(beta_1_)
        beta_2 = random.choice(beta_2_)
        epsilon = random.choice(epsilon_)
    ...
    plot_param_vs_score("power_t")
    plot_param_vs_score("beta_1")
    plot_param_vs_score("beta_2")
...
def evaluate_model(
    ...
    estimator = MLPRegressor(
        hidden_layer_sizes=int(best_parameters["hidden_layer_sizes"]),
        activation="tanh",
        solver="adam",
        ...
        early_stopping=False,
        validation_fraction=0.1,
        beta_1=best_parameters["beta_1"],
        beta_2=best_parameters["beta_2"],
...
def model_predictions(
    ...
    for i in range(0, num_iterations):
        ...
        model_prediction_storage["pred_extreme_Sedep"][i] = y_pred_extreme_Sedep
    ...
    post_process_predictions("pred_extreme_Sedep", lat_lon_future)
